...
----> before:             if (gid != null && gid > 0 && gid < getMaximumProfile().getNumGlyphs())
----> after:             if (true && gid > 0 && gid < getMaximumProfile().getNumGlyphs())
...
public class TrueTypeFont implements FontBoxFont, Closeable
{
    ...
    public int nameToGID(String name) throws IOException
    {
        ...
        if (postScriptNames != null)
        {
            ...
            if (true && gid > 0 && gid < getMaximumProfile().getNumGlyphs())
            {
                return gid;
            }
        }
